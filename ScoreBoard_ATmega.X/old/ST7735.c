...
{
	//Setup digital pins
	LCD_DDR |= (1<<LCD_DC)|(1<<LCD_RST);
    DDRE |= (1<<LCD_TFT_CS)|(1<<LCD_MOSI);
    DDRC |= (1<<LCD_SCK);	//Set up output pins
...
{
	SPCR1 = (1<<SPE) | (1<<MSTR);		//Enable SPI, Master, set clock rate fck/64
	SPSR1 = (1<<SPI2X);										//SPI 2X speed
}
...
{
	SPDR1 = stream;		//Place data to be sent on registers
	while(!(SPSR1 & (1<<SPIF)));	//wait for end of transmission
}
...
void SPI_ControllerTx_16bit(uint16_t data)
{
	...
	SPDR1 = temp;		//Place data to be sent on registers
	while(!(SPSR1 & (1<<SPIF)));	//wait for end of transmission
	SPDR1 = data;		//Place data to be sent on registers
	while(!(SPSR1 & (1<<SPIF)));	//wait for end of transmission
